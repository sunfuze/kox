...
module.exports = function (app) {
  ...
  app.loadApis = function (apiCfs, settings = {}) {
    ...
    const router = settings.basePath !== '/'
      ? new Router({ prefix: settings.basePath })
      : new Router()
    // add swagger route
    ...
    forEach(apis, api => {
      validateAPI(api)
      route(app, router, api, settings.validateResponse)
    })
...
function route (app, router, api, validateResponse = false) {
  let action
  ...
  validateResponse = api.validateResponse || validateResponse
  router[api.method](formatPath(api.path), function* (next) {
    ...
    if (validateResponse && Hoek.reach(api, 'responses')) {
