...
class Controller {
  ...
  action (name, fn) {
    ...
    assert(Utilities.isString(name) && name.length, `action must have a name, controller: ${this.name}`)
    assert(Utilities.isFunction(fn), `action handle must be a function, controller: ${this.name}, action: ${name}`)
  ...
  invoke (name) {
    assert(this._actions[name], `Controller[${this.name}] don't have an action named: ${name}`)
    ...
    function wrapAction (fn) {
      return function* (next) {
        this.body = (Utilities.isGenerator(fn) ? (yield fn(this)) : fn(this)) || this.body
        yield* next
      }
    }
    ...
    function formatHook (hook) {
      return function* (next) {
        ...
        if (this.status && this.status > 299 && this.status < 400) {
          return
        }
